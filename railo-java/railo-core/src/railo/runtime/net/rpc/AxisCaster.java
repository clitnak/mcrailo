...
public final class AxisCaster {
    ...
    private static Map<String,Object> toMap(TypeMapping tm,Object value, Class targetClass, Set<Object> done,String namespace) throws PageException {
        ...
        while(it.hasNext()) {
        	e = it.next();
            trg.put(e.getKey().getString(),_toAxisType(tm,null,null,e.getValue(),targetClass,done,namespace));
    		...
    		try{
    			if(attemptConversionToComponent) {		
	        		...
	        		for(int i=0;i<props.length;i++){
	        			prop=props[i];
	        			try{
	        				cw.set(pc, KeyImpl.init(prop.getName()), toRailoType(pc,Reflector.callGetter(value, prop.getName()),attemptConversionToComponent));
	        			}
	        			catch(PageException pe){
	        				pe.printStackTrace();
	        			}
